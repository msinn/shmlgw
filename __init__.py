...
class mlgwBase():
    ...
    def __init__(self):
        ...
        logger.info("mlgw: beo4commanddict=" + str(beo4commanddict))
        logger.info("mlgw: reverse_beo4commanddict=" + str(reverse_beo4commanddict))
        logger.info("mlgw: selectedsourcedict=" + str(selectedsourcedict))
        logger.info("mlgw: reverse_selectedsourcedict=" + str(reverse_selectedsourcedict))
        logger.info("mlgw: speakermodedict=" + str(speakermodedict))
        logger.info("mlgw: reverse_speakermodedict=" + str(reverse_speakermodedict))
    ...
    def OpenConnection(self, host=None, port=None):
        ...
        try:
            ...
            logger.error("mlgw: Error resolving '%s': %s" % (host, e))
            return
        ...
        try:
            ...
            logger.error("mlgw: Error opening connection to %s: %s" % (self._tcpip, e))
            return
        if self._tcpip != host:
            logger.info("mlgw: Opened connection to ML Gateway '" + host + "' on IP " + self._tcpip + ":" + str( self._port ))
        else:
            logger.info("mlgw: Opened connection to ML Gateway on IP " + self._tcpip + ":" + str( self._port ))
        self.connected = 1
        return
    ...
    def CloseConnection(self):
        ...
        logger.info("mlgw: Closed connection to ML Gateway")
        return
    ...
    def SendCommand(self, type, payload ):
        ...
        if self.telegramlogging:
            ...
            logger.log(loglevel, "mlgw: >SENT: " + _getpayloadtypestr( type ) + ": " + _getpayloadstr( self._telegram ))  # debug
        return
...
#        self._payload.append(0x00)             # byte[3] Link
    ...
    def ReceiveCommand(self):
        ...
        try:
            ...
            logger.error("mlgw: KeyboardInterrupt, terminating...")
            self.CloseConnection()
            sys.exit(1)

        self._payloadstr = _getpayloadstr( self._mlgwdata )
        if self._mlgwdata[0] != 0x01:
            logger.error("mlgw: Received telegram with SOH byte <> 0x01")
        if self._mlgwdata[3] != 0x00:
            logger.error("mlgw: Received telegram with spare byte <> 0x00")
        if self.telegramlogging:
            loglevel = loglevel_receivedtelegrams
            if self._mlgwdata[1] == 0x37:
                loglevel = loglevel_keepalivetelegrams
            logger.log(loglevel, "mlgw: <RCVD: '" + _getpayloadtypestr( self._mlgwdata[1] ) + "': " + str(self._payloadstr))  # debug
    ...
    def GetSerial(self):
      ...
      # Request serial number
        ...
        logger.warning("mlgw: Serial number of ML Gateway is " + self._serial)  # info
        return
    ...
    def Login(self, username, password):
        ...
        if result == 0x31:
            logger.info("mlgw: Login required for this ML Gateway")
            ...
            if self._wrkstr == 'FAIL':
                logger.error("mlgw: Login not successful, user / password combination is not valid!")
                self.CloseConnection()
            else:
              logger.info("mlgw: Login successful, connection established")
        else:
            logger.info("mlgw: Connection established")
        return
    ...
    def run( self ):
        ...
        while self._mlgwbase.connected: 
            try:
                ...
                self._payloadstr = _getpayloadstr( self._mlgwdata )
                if self._mlgwdata[0] != 0x01:
                    logger.error("Received telegram with SOH byte <> 0x01")
                if self._mlgwdata[3] != 0x00:
                    logger.error("Received telegram with spare byte <> 0x00")
...
#                    logger.log(logging.WARNING, "mlgw: loglevel="+str(loglevel)+", log_telegrams="+str(log_telegrams))
                    if log_telegrams > 1:
                        logger.log(loglevel, "<RCVD: " + _getpayloadtypestr( self._mlgwdata[1] ) + ": " + str(self._payloadstr))  # info
                if (self._mlgwdata[0] == 0x01) and (self._mlgwdata[3] == 0x00) and (self._mlgwdata[1] != 0x37):
                    ...
                    if (telegram_handled == False) and (log_telegrams >= 1):
                        logger.log(loglevel, "UNHANDLED: " + _getpayloadtypestr( self._mlgwdata[1] ) + ": " + str(self._payloadstr))  # info
            if timeoutcount > 12:
                logger.error("No answer received to mlgw-ping")
            if timeoutcount > 11:
    ...
    def processtelegram( self ):
        ...
        if self._mlgwdata[1] == 0x04:
            ...
            if self._mlgwdata[5] == 0x01:
                ...
                if item != None:
#                    logger.warning('processtelegram: L&C telegram: room='+str(self._mlgwdata[4])+', LC=LIGHT, command='+cmd+', item='+str(item))
...
#                    logger.warning('processtelegram: L&C telegram: room='+str(self._mlgwdata[4])+', LC=LIGHT, command='+cmd+', item='+str(item))
            ...
            # CONTROL telegram
                ...
                if item != None:
#                    logger.warning('processtelegram: L&C telegram: room='+str(self._mlgwdata[4])+', LC=CONTROL, command='+cmd+', item='+str(item))
...
#                    logger.warning('processtelegram: L&C telegram: room='+str(self._mlgwdata[4])+', LC=CONTROL, command='+cmd+', item='+str(item))
        ...
        ##### SOURCE STATUS telegrams
            ...
            if item != None:
#                logger.warning('processtelegram: Source Status telegram: mln='+str(self._mlgwdata[4])+', source='+source+', item='+str(item))
...
#                logger.warning('processtelegram: Source Status telegram: mln='+str(self._mlgwdata[4])+', source='+source+', item='+str(item))
        ...
        ##### PICT&SND STATUS telegrams
            ...
            if item != None:
#                logger.warning('processtelegram: Source Status telegram: mln='+str(self._mlgwdata[4])+', speakermode='+speakermode+', item='+str(item))
...
class mlgw():
    ...
    def __init__(self, smarthome, host='mlgw.local', port=9000, username='mlgw', password='mlgw', rooms=[], mlns=[], log_mlgwtelegrams='0'):
        ...
        logger.debug("mlgw: mlgw.__init__()")
        ...
        if rooms == []:
            logger.warning("mlgw: No rooms defined")
        else:
            roomdict = ast.literal_eval(rooms)
            logger.info("mlgw: Rooms defined: " + str(roomdict))
            reverse_roomdict = {v.upper(): k for k, v in roomdict.items()}
            logger.info("mlgw: reverse_roomdict=" + str(reverse_roomdict))

        if mlns == []:
            logger.warning("mlgw: No MLNs defined")
        else:
            mlndict = ast.literal_eval(mlns)
            logger.info("mlgw: MLNs defined: " + str(mlndict))
            reverse_mlndict = {v.upper(): k for k, v in mlndict.items()}
            logger.info("mlgw: reverse_mlndict=" + str(reverse_mlndict))
        self.alive = False
    ...
    def run(self):
        ...
        logger.info("listenerlightdict: " + str(listenerlightdict))
        logger.info("listenercontroldict: " + str(listenercontroldict))
        logger.info("listenersourcestatusdict: " + str(listenersourcestatusdict))
        logger.info("listenerspeakermodedict: " + str(listenerspeakermodedict))

        logger.debug("mlgw.run()")
        self.alive = True
    ...
    #
    def stop(self):
        logger.debug("mlgw.stop()")
        self._mlgwbase.CloseConnection()
        if self.alive:
            logger.info("mlgw: Waiting for threads to stop")
            self.listener.join()
        self.alive = False
        ...
        if 'mlgw_send' in item.conf:
            ...
            if mln == None:
                try:
                    ...
                    logger.error("mlgw: parse item: {0}".format(item) + " - mlgw_mln is not numeric")
                    return None
            ...
            if item.conf['mlgw_send'].upper() == 'CMD':
                ...
                logger.info("mlgw: parse item: {0}".format(item) + ", type=" + item._type + ", item.conf=" + str(item.conf) )
                ...
                if item._type == 'num':
                    ...
                    logger.error("mlgw: parse item: {0}".format(item) + " - 'mlgw_send' is 'ch' but 'type' is not 'num'")
                    return None
            ...
            if item.conf['mlgw_listen'].upper() == 'LIGHT':
                ...
                if room == None:
                    try:
                        ...
                        logger.error("mlgw: parse item: {0}".format(item) + " - mlgw_room is not numeric")
                        return None
...
#            mln = int(item.conf['mlgw_mln'])
            ...
            if mln == None:
                try:
                    ...
                    logger.error("mlgw: update_channel: {0}".format(item) + " - mlgw_mln is not numeric")
                    return None
            logger.warning("mlgw: update channel: {0}".format(item.id())+", value="+ str(item()) +", (MLN="+str(mln)+")")   # info
            channel = item()
            wrk = channel
            if wrk // 100 != 0:
...
#            mln = int(item.conf['mlgw_mln'])
            ...
            if mln == None:
                try:
                    ...
                    logger.error("mlgw: update_item: {0}".format(item) + " - mlgw_mln is not numeric")
                    return None
            if item._type == 'str':
                ...
                cmd = item()
            logger.warning("mlgw: update item: {0}".format(item.id())+", value="+ str(item()) +", (MLN="+str(mln)+", cmd="+str(cmd)+")")   # info
